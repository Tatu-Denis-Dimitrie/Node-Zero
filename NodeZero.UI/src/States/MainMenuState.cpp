...
MainMenuState::MainMenuState(std::function<void(GameState)> stateChangeCallback)
    ...
    auto playButton = std::make_unique<Button>(centerX, startY, buttonWidth, buttonHeight, "Play");
    playButton->SetColors(Color{70, 130, 180, 255}, Color{100, 160, 210, 255}, Color{40, 100, 150, 255}, WHITE);
    playButton->SetOnClick([this]() { m_StateChangeCallback(GameState::Playing); });
    m_Menu->AddWidget(std::move(playButton));

    auto upgradesButton = std::make_unique<Button>(centerX, startY + buttonHeight + buttonSpacing, buttonWidth, buttonHeight, "Upgrades");
    upgradesButton->SetColors(Color{50, 150, 50, 255}, Color{80, 180, 80, 255}, Color{30, 120, 30, 255}, WHITE);
    upgradesButton->SetOnClick([this]() { m_StateChangeCallback(GameState::Settings); });
    m_Menu->AddWidget(std::move(upgradesButton));

    auto quitButton = std::make_unique<Button>(centerX, startY + (buttonHeight + buttonSpacing) * 2, buttonWidth, buttonHeight, "Quit");
    quitButton->SetColors(Color{180, 70, 70, 255}, Color{210, 100, 100, 255}, Color{150, 40, 40, 255}, WHITE);
    quitButton->SetOnClick([this]() { m_StateChangeCallback(GameState::Quit); });
